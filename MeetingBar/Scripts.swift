...
/**
 * we will schedule a common task to check if we have to execute the apple script for events.
...
func scheduleRunScriptForMeetingStart() {
    let activity = NSBackgroundActivityScheduler(identifier: "leits.MeetingBar.runscriptfornextmeeting")
...
func createParameters(event: EKEvent) -> NSAppleEventDescriptor {
    ...
    if let (service, meetingUrl) = getMeetingLink(event) {
        parameters.insert(NSAppleEventDescriptor(string: meetingUrl.absoluteString), at: 0)
        parameters.insert(NSAppleEventDescriptor(string: service!.rawValue), at: 0)
    } else {
